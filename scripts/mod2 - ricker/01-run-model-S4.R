...
# generate initials
init <- function() {
  list(a = matrix(rnorm(4, 0, 10), ncol = 2),
       b = matrix(runif(4, 0, 1), ncol = 2),
       c = matrix(rnorm(4, 0, 10), ncol = 2),
       tau = runif(1, 0, 1))
}
inits_list <- list(init(), init(), init())
load("scripts/mod2 - ricker/inits/inits_S4.Rdata")
# Initiate model
jm <- jags.model("scripts/mod2 - ricker/ricker_S4.JAGS",
                 inits = inits_list,
                 data = data_list,
                 n.chains = 3)
...
saved_state[[1]]
# save(saved_state, file = "scripts/mod2 - ricker/inits/inits_S4.Rdata")
...
params_plot_df |> 
  filter(term  %in% c("peak.x", "peak.y")) |> 
  mutate(param = "param") |> 
  ...
  geom_point(aes(color = period),
             size = 2.5,
             position = position_dodge(width = 0.9)) +
  geom_point(data = params_sig_df |> filter(term  %in% c("peak.x", "peak.y")),
...
ggplot() +
  ...
  geom_pointrange(data = preds, 
                  aes(x = days_since_pulse, y = pred.mean,
                      ymin = pred.lower,
                      ymax = pred.upper),
                  color = "gray50", alpha = 0.25) +
  geom_line(data = preds, 
            aes(x = days_since_pulse, y = pred.mean),
            color = "gray50", lty = 2) +
  geom_errorbarh(data = peaks_df,
                 aes(xmin = pred.lower_x, xmax = pred.upper_x,
                     y = pred.mean_y),
                 height = 0) +   
  geom_errorbar(data = peaks_df,
                aes(x = pred.mean_x,
