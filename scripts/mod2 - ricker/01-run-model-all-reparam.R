...
df <- wp_rwc |> 
  ...
  mutate(period = factor(period, levels = c("predawn", "midday")),
         ...
         plotID = factor(ID, levels = unique(df$ID)))

#  calculate  trt associated with each plot
trt_df <- df |> 
  group_by(plotID) |> 
  summarize(trt = unique(trt_s))
str(trt_df)
...
# data list
data_list <- list(Y = df$scaled_value, # response variable, wp or rwc, scaled between 0 and 1
                 t = df$days_since_pulse, # predictor, days since pulse, 0-21,
                 plot = df$plotID, 
                 ...
                 N = nrow(df),
                 Nplot = nrow(trt_df),
                 Ntrt = length(unique(trt_df$trt)))
str(data_list)
# generate initials
init <- function() {
  list(mu.Lt.peak = array(runif(12, 0, log(20)), dim = c(3,2,2)),
       mu.y.peak = array(rnorm(12, 0, 10), dim = c(3,2,2)),
...
# Initiate model
jm <- jags.model("scripts/mod2 - ricker/ricker_all_reparam.JAGS",
                 inits = saved_state[[2]],
                 data = data_list,
                 n.chains = 3)
# jm <- jags.model("scripts/mod2 - ricker/ricker_all_reparam.JAGS",
...
#                  n.chains = 3)
update(jm, 10000)         
dic.samples(jm, 1000) # pD = 27

# parameters to monitor
params_vec <- c("mu.t.peak", "mu.y.peak", "Sigs",
                "tau", "mu.Lt.peak", "sig.Lt.peak", "sig.y.peak",
                "deviance", "Dsum", "R2")
...
mcmcplot(jm_coda, parms = c("Dsum", "R2", "deviance",
                            "mu.t.peak", "mu.y.peak", "Sigs"))
caterplot(jm_coda, regex = "^mu.t.peak\\[1", reorder = FALSE)
caterplot(jm_coda, regex = "^mu.t.peak\\[2", reorder = FALSE)
caterplot(jm_coda, regex = "^mu.t.peak\\[3", reorder = FALSE)

caterplot(jm_coda, regex = "^mu.y.peak\\[1", reorder = FALSE)
caterplot(jm_coda, regex = "^mu.y.peak\\[2", reorder = FALSE)
caterplot(jm_coda, regex = "^mu.y.peak\\[3", reorder = FALSE)

caterplot(jm_coda, regex = "Sigs", reorder = FALSE)
caterplot(jm_coda, regex = "Dsum", reorder = FALSE)
caterplot(jm_coda, regex = "R2", reorder = FALSE)
...
gel$psrf |> 
  data.frame() |> 
  tibble::rownames_to_column() |> 
  filter(grepl("^mu.y.peak", rowname))

gel$psrf |> 
  data.frame() |> 
  tibble::rownames_to_column() |> 
  filter(grepl("^mu.t.peak", rowname))

# Save state
final <- initfind(jm_coda, OpenBUGS = FALSE)
final[[1]]
saved_state <- removevars(final, variables = c(1:3, 5))
saved_state[[1]]
