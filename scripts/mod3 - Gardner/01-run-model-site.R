...
# Initials
inits <- function() {
  list(inv.b.mu = rnorm(1, 0, 10),
       log.a.mu = rnorm(1, 0, 10),
...
load("scripts/mod3 - Gardner/inits/inits_site.Rdata")
# Compile model
jm <- jags.model(file = "scripts/mod3 - Gardner/Gardner_site.JAGS",
                 inits = saved_state[[2]],
                 data = data_list,
                 n.chains = 3)
# update(jm, 10000)
...
caterplot(coda_params, parms = c("log.a.mu", "inv.b.mu"), reorder = FALSE)
caterplot(coda_params, parms = c("a.mu", "b.mu"), reorder = FALSE)
...
# saved_state[[1]]
# save(saved_state, file = "scripts/mod3 - Gardner/inits/inits_site.Rdata")

save(coda_params, file = "scripts/mod3 - Gardner/coda/coda_params_site.Rdata")
...
site_p <- param_sum |> 
  filter(term %in% c("inv.b.mu", "log.a.mu"))

x <- seq(log(min(vwc$mean, na.rm = TRUE)) - log(0.05),
         log(max(vwc$mean, na.rm = TRUE)) - log(0.05),
         by = 0.01)
invb <- site_p$pred.mean[1]
loga <- site_p$pred.mean[2]
